/*
 * SPCOMServices - Work with SharePoint's Web Services using jQuery
 * Version 0.6.2
 * @requires jQuery v1.4.2 or greater
 *
 * Copyright (c) 2009-2010 Sympraxis Consulting LLC
 * Examples and docs at:
 * http://SPCOMServices.codeplex.com
 ...
 * @type jQuery
 * @name SPCOMServices
 * @category Plugins/SPCOMServices
...
(function ($) {
	...
	$.fn.SPCOMServices = function(options) {

		// If there are no options passed in, use the defaults.  Extend replaces each default with the passed option.
		var opt = $.extend({}, $.fn.SPCOMServices.defaults, options);
		...
		if(opt.webURL.charAt(opt.webURL.length - 1) === SLASH) {
			...
			ajaxURL = $().SPCOMServices.SPGetCurrentSite() + SLASH + ajaxURL;
		}
	...
	}; // End $.fn.SPCOMServices

	// Defaults added as a function in our library means that the caller can override the defaults
	// for their session by calling this function.  Each operation requires a different set of options;
	// we allow for all in a standardized way.
	$.fn.SPCOMServices.defaults = {
	...
	$.fn.SPCOMServices.SPGetCurrentSite = function() {
	...
	$.fn.SPCOMServices.SPCascadeDropdowns = function(options) {
		...
		var opt = $.extend({}, {
			...
			listName: $().SPCOMServices.SPListNameFromUrl(),		// The list the form is working with. This is useful if the form is not in the list context.
		...
		if(parentSelect.Obj.html() === null && opt.debug) {errBox("SPCOMServices.SPCascadeDropdowns", "parentColumn: " + opt.parentColumn, "Column not found on page");return;}

		// Find the child column's select (dropdown)
		var childSelect = new dropdownCtl(opt.childColumn);
		if(childSelect.Obj.html() === null && opt.debug) {errBox("SPCOMServices.SPCascadeDropdowns", "childColumn: " + opt.childColumn, "Column not found on page");return;}
		...
		if(opt.simpleChild === true && childSelect.Type === "C") {
			$().SPCOMServices.SPComplexToSimpleDropdown({
	...
	}; // End $.fn.SPCOMServices.SPCascadeDropdowns
	...
	function cascadeDropdown(opt, childSelect) {
		...
		var childColumnStatic = $().SPCOMServices.SPGetStaticFromDisplay({
		...
		$().SPCOMServices({
			...
			completefunc: function(xData, Status) {
				$(xData.responseXML).find("faultcode").each(function() {
					if(opt.debug) {
						errBox("SPCOMServices.SPCascadeDropdowns",
	...
	$.fn.SPCOMServices.SPComplexToSimpleDropdown = function(options) {
		...
		if(columnSelect.Obj.html() === null && opt.debug) {
			errBox("SPCOMServices.SPComplexToSimpleDropdown",
	...
	}; // End $.fn.SPCOMServices.SPConvertToSimpleDropdown


	// Function to display related information when an option is selected on a form.
	$.fn.SPCOMServices.SPDisplayRelatedInfo = function(options) {
		...
		var columnSelect = new dropdownCtl(opt.columnName);
		if(columnSelect.Obj.html() === null && opt.debug) {
			errBox("SPCOMServices.SPDisplayRelatedInfo",
				"columnName: " + opt.columnName,
				"Column not found on page");
			return;
		}
	...
	}; // End $.fn.SPCOMServices.SPDisplayRelatedInfo
	...
	$.fn.SPCOMServices.SPFilterDropdown = function(options) {
		var opt = $.extend({}, {
		...
		var columnSelect = new dropdownCtl(opt.columnName);
		if(columnSelect.Obj.html() === null && opt.debug) {
			errBox("SPCOMServices.SPFilterDropdown", "columnName: " + opt.columnName, "Column not found on page");
			return;
		}
		...
		$().SPCOMServices({
			...
			completefunc: function(xData, Status) {
				$(xData.responseXML).find("faultcode").each(function() {
					if(opt.debug) {
						errBox("SPCOMServices.SPFilterDropdown",
	...
	}; // End $.fn.SPCOMServices.SPFilterDropdown


	// Utility function to show the results of a Web Service call formatted well in the browser.
	$.fn.SPCOMServices.SPDebugXMLHttpResult = function(options) {
		...
		// Handle child nodes
			...
			for (i = 0;i < opt.node.childNodes.length; i++) {
				outString += $().SPCOMServices.SPDebugXMLHttpResult({
		...
		return outString;
	}; // End $.fn.SPCOMServices.SPDebugXMLHttpResult

	// Function which returns the account name for the current user in DOMAIN\username format
	$.fn.SPCOMServices.SPGetCurrentUser = function(options) {
		...
		$.ajax({
			...
			url: $().SPCOMServices.SPGetCurrentSite() + "/_layouts/userdisp.aspx?Force=True&" + new Date().getTime(),
			complete: function (xData, Status) {
	...
	}; // End $.fn.SPCOMServices.SPGetCurrentUser
	...
	// by Waldek Mastykarz
	$.fn.SPCOMServices.SPLookupAddNew = function(options) {
		...
		if(lookupSelect.Obj.html() === null && opt.debug) { errBox("SPCOMServices.SPLookupAddNew", "lookupColumn: " + opt.lookupColumn, "Column not found on page");return;}
		...
		$().SPCOMServices({
			operation: "GetList",
			async: false,
			listName: $().SPCOMServices.SPListNameFromUrl(),
			completefunc: function (xData, Status) {
		...
		if(lookupListUrl.length === 0 && opt.debug) {
			errBox("SPCOMServices.SPLookupAddNew",
	...
	}; // End $.fn.SPCOMServices.SPLookupAddNew
	...
	// between list forms
	$.fn.SPCOMServices.SPGetLastItemId = function(options) {
		...
		$().SPCOMServices({
			...
			async: false,
			userLoginName: (opt.userAccount !== "") ? opt.userAccount : $().SPCOMServices.SPGetCurrentUser(),
			completefunc: function (xData, Status) {
	...
	$.fn.SPCOMServices.SPRequireUnique = function (options) {
		...
		var queryStringVals = $().SPCOMServices.SPGetQueryString();
		var thisID = queryStringVals.ID;
		var thisList = $().SPCOMServices.SPListNameFromUrl();
		...
		var columnDisplayName = $().SPCOMServices.SPGetDisplayFromStatic({
			listName: thisList,
	...
	}; // End $.fn.SPCOMServices.SPRequireUnique

	// This function returns the DisplayName for a column based on the StaticName.
	$.fn.SPCOMServices.SPGetDisplayFromStatic = function (options) {
		...
		return displayName;
	}; // End $.fn.SPCOMServices.SPGetDisplayFromStatic

	// This function returns the StaticName for a column based on the DisplayName.
	$.fn.SPCOMServices.SPGetStaticFromDisplay = function (options) {
		...
		return staticName;
	}; // End $.fn.SPCOMServices.SPGetStaticFromDisplay
	...
	$.fn.SPCOMServices.SPRedirectWithID = function (options) {
		...
		if(typeof queryStringVals.ID === 'undefined') {
			lastID = $().SPCOMServices.SPGetLastItemId({
				listName: thisList
			});
	...
	}; // End $.fn.SPCOMServices.SPRedirectWithID
	...
	$.fn.SPCOMServices.SPSetMultiSelectSizes = function (options) {
		...
		$("#" + cloneId).remove();
	}; // End $.fn.SPCOMServices.SPSetMultiSelectSizes

	// Does an audit of a site's list forms to show where script is in use.
	$.fn.SPCOMServices.SPScriptAudit = function (options) {
													...
													for(i=0; i < formTypes.length; i++) {
														...
														$(formUrls).find(formTypes[i][0]).each(function() {
															SPScriptAuditPage(opt, listXml, "Form", this.nodeName,
																((opt.webURL.length > 0) ? opt.webURL : $().SPCOMServices.SPGetCurrentSite()) + SLASH + $(this).text());
															formTypes[i][2] = true;
														});
	...
	}; // End $.fn.SPCOMServices.SPScriptAudit
	...
	$.fn.SPCOMServices.SPArrangeChoices = function (options) {
		...
		$().SPCOMServices({
			operation: "GetList",
			async: false,
			listName: (opt.listName.length > 0) ? opt.listName : $().SPCOMServices.SPListNameFromUrl(),
			completefunc: function(xData, Status) {
				$(xData.responseXML).find("Fields").each(function() {
					$(this).find("Field[DisplayName='" + opt.columnName + "']").each(function() {
	...
	}; // End $.fn.SPCOMServices.SPArrangeChoices

	// Provide suggested values from a list for in input column based on characters typed
	$.fn.SPCOMServices.SPAutocomplete = function (options) {
		...
		if(columnObj.html() === null && opt.debug) {
			errBox("SPCOMServices.SPAutocomplete",
				"columnName: " + opt.columnName,
	...
	}; // End $.fn.SPCOMServices.SPAutocomplete

	// Get the Query String parameters and their values and return in an array
	$.fn.SPCOMServices.SPGetQueryString = function () {
		var i;
		var queryStringVals = {};
	...
	}; // End $.fn.SPCOMServices.SPGetQueryString
	...
	$.fn.SPCOMServices.SPListNameFromUrl = function () {
		...
		// Return the GUID (ID)
		return thisList;
	}; // End $.fn.SPCOMServices.SPListNameFromUrl
	...
	$.fn.SPCOMServices.SPUpdateMultipleListItems = function (options) {
	...
	}; // End $.fn.SPCOMServices.SPUpdateMultipleListItems
...
////// PRIVATE FUNCTIONS ////////
	...
	function modalBox(msg) {
		...
		$("#aspnetForm").parent().append("<div id='SPCOMServices_msgBox' style=" + boxCSS + ">" + msg);
		var height = $("#SPCOMServices_msgBox").height();
		var width = $("#SPCOMServices_msgBox").width();
		var leftVal = ($(window).width() / 2) - (width / 2) + "px";
		var topVal = ($(window).height() / 2) - (height / 2) - 100 + "px";
		$("#SPCOMServices_msgBox").css({border:'5px #C02000 solid', left:leftVal, top:topVal}).show().fadeTo("slow", 0.75).click(function () {
	...
	function genContainerId(funcname, columnName) {
		return funcname + "_" + $().SPCOMServices.SPGetStaticFromDisplay({
			listName: $().SPCOMServices.SPListNameFromUrl(),

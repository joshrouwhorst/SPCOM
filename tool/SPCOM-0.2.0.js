var SPCOM = (function(){
    ...
    var UI = (function(){
        ...
        function Column(id){
            ...
            this.startNew = function(itemArr, linked){
        ...
        var tabs = (function(){
            ...
            var clear = function(){
                $("#viewArea").html(viewTabs + "<div id='viewLogo'><table id='logoTable'><tr><td><img src='http://joshrouwhorst.com/sp/imgs/v0.2/spCom-150.png' /><div class='spcomVersion'>v" + version + "</div></td></tr></table><table id='logoMenu'><tr><td onclick='SPCOM.link(\"" + siteUrl +
                                    ...
                                    "<tr><td></td><td onclick='SPCOM.startLoading()'>Refresh</td><td></td></tr></table></div>");
                tabsArr = [];
                        ...
                        "<tr>" +
                            ...
                            "<td class='listTable-main'><div class='listMenu'>" +
                                ...
                                "<table class='listStats'>" + 
                                    ...
                                    "<tr><td>Primary Key: " + ((field.primary) ? "True" : "False") + "</td><td>ColName: " + field.colName + "</td></tr>" +
                                    ...
                                    "<td class='listTable-side'>" +
                                        "<div class='listMenu'>" +
                                            "<table class='listMenu'>" +
                                                ...
                                                "<tr><td onclick='SPCOM.testQuery.init(\"" + list.title + "\")'>Test Query</td></tr>" +
                                            "</table>" +
                                        "</div>" +
                                    "</td>" +
                                    "<td class='listTable-main'><div class='listMenu'>" +
    ...
    var testQuery = (function(){
        ...
        var run = function(){
            var query = $("#testQuery textarea").val();
            $().SPCOMServices({
                ...
                completefunc: function(xData, status){
                    ...
                    for (var tag in queryArr){
                        queryObj.find(tag).each(function(){
                            ...
                            for (var i = 0; i < results.items.length; i++){
                                if (tag === "Eq" && results.items[i][attr] !== undefined && results.items[i][attr] === value){
                                    itemArr.push(results.items[i]);
                                }
                                else if (tag === "Neq" && results.items[i][attr] !== undefined && results.items[i][attr] !== value){
                                    itemArr.push(results.items[i]);
                                }
                                else if (tag === "Contains" && results.items[i][attr] !== undefined && results.items[i][attr].indexOf(value) > -1){
                                    itemArr.push(results.items[i]);
                                }
                                else if (tag === "Gt" && results.items[i][attr] !== undefined && results.items[i][attr] > value){
                                    itemArr.push(results.items[i]);
                                }
                                else if (tag === "Lt" && results.items[i][attr] !== undefined && results.items[i][attr] < value){
                                    itemArr.push(results.items[i]);
                                }
                                else if (tag === "Geq" && results.items[i][attr] !== undefined && results.items[i][attr] >= value){
                                    itemArr.push(results.items[i]);
                                }
                                else if (tag === "Leq" && results.items[i][attr] !== undefined && results.items[i][attr] <= value){
                                    itemArr.push(results.items[i]);
                                }
                                else if (tag === "BeginsWith" && results.items[i][attr] !== undefined && results.items[i][attr].indexOf(value) === 0){
                                    itemArr.push(results.items[i]);
                                }
                            }
                    ...
                    $("#testQueryResults thead").html(html);
                    html = "";
                    for (var i = 0; i < results.items.length; i++){
                        html += "<tr>";
                        for (var id in list.fields){
                    ...
                    $("#testQueryResults tbody").html(html);
                    
                    html = "<table class='testQueryStats' cellspacing='0px'><tr><td colspan='2'><span class='label'>Item Count:</span> " + results.items.length + "</td></tr>";
                    var i = -1; 
                    for (var tag in queryArr){
    ...
    function init(){
        ...
        */
        ...
        var testQueryHtml = "<div id='testQuery'><table class='testQuery'><tr><td class='testQuery'><textarea class='testQuery'></textarea></td><td><input type='button' value='Run' class='testQuery' onclick='SPCOM.testQuery.run()' /></td></tr></table><div id='testQueryResults'><div id='testQueryStats'></div><div id='testQueryRecords'><table><thead></thead><tbody></tbody></table></div></div></div>";
        $("body").append(loadingHtml + spcomHtml + testQueryHtml);*/
        var spcomHtml = "<div id='spcom'><div id='columns'><table id='columnsTable'><tr><td><div id='leftColumn'></div></td><td><div id='middleColumn'></div></td><td><div id='rightColumn'></div></td></tr></table></div><div id='viewArea'><div id='viewTabs'></div><div id='currentView'></div></div></div><script type='text/javascript'>SPCOM.startLoading();</script>";
        var testQueryHtml = "<div id='testQuery'><table class='testQuery'><tr><td class='testQuery'><textarea class='testQuery'></textarea></td><td><input type='button' value='Run' class='testQuery' onclick='SPCOM.testQuery.run()' /></td></tr></table><div id='testQueryResults'><div id='testQueryStats'></div><div id='testQueryRecords'><table cellspacing='0px'><thead></thead><tbody></tbody></table></div></div><div id='testQueryClose'><input type='button' value='Close' onclick='SPCOM.testQuery.close()' /></div></div>";
        $("body").append(spcomHtml + testQueryHtml);
    }
